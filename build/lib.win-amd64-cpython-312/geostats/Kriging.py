...
import numpy as np
from geostats import build_Mrho
...
class Site:
	...
	"""
	latitude: Sites latitude (deg).
	longitude: Sites longitude (deg).
...
class Kriging:
	...
	epi_lat/epi_lon: Coordinates of the epicenter (deg).
	...
	def Krige(self,Ktype):
		...
		if self.Mrho_sam is None:
			...
			if self.model.m_type.lower() == 'e':
				self.Mrho_sam = build_Mrho.MrhoE_sam2sam(self.sam.latitude,self.sam.longitude,self.model.L_E,self.model.gamma_E)
				self.Mrho_uns = build_Mrho.MrhoE_sam2uns(self.sam.latitude,self.sam.longitude,self.uns.latitude,self.uns.longitude,self.model.L_E,self.model.gamma_E)

			elif self.model.m_type.lower() == 'ea':
				self.Mrho_sam = build_Mrho.MrhoEA_sam2sam(self.sam.latitude,self.sam.longitude,self.epi_lat,self.epi_lon,self.model.L_E,self.model.gamma_E,self.model.L_A)
				self.Mrho_uns = build_Mrho.MrhoEA_sam2uns(self.sam.latitude,self.sam.longitude,self.epi_lat,self.epi_lon,self.uns.latitude,self.uns.longitude,self.model.L_E,self.model.gamma_E,self.model.L_A)

			elif self.model.m_type.lower() == 'eas':
				self.Mrho_sam = build_Mrho.MrhoEAS_sam2sam(self.sam.latitude,self.sam.longitude,self.sam.Vs30,self.epi_lat,self.epi_lon,self.model.L_E,self.model.gamma_E,self.model.L_A,self.model.L_S,self.model.weight)
				self.Mrho_uns = build_Mrho.MrhoEAS_sam2uns(self.sam.latitude,self.sam.longitude,self.sam.Vs30,self.epi_lat,self.epi_lon,self.uns.latitude,self.uns.longitude,self.uns.Vs30,self.model.L_E,self.model.gamma_E,self.model.L_A,self.model.L_S,self.model.weight)

		noise = 1e-4
		self.Mrho_sam -= (noise)*np.identity(len(self.Mrho_sam))
		...
		if Ktype.lower() == 'ordinary':
			...
			k = np.linalg.solve(Mcov_sam,Mcov_uns)
			w  = k[0:M,0:N].T
			mu = w@self.sam.value
